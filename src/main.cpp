...
int main() {
    ...
    auto material_wall = make_shared<DiffuseMaterial>(color(0.9, 0.9, 0.9));
    auto material_table = make_shared<DiffuseMaterial>(color(0.351, 0.127, 0.047));
    auto material_plate = make_shared<DiffuseMaterial>(color(0.9, 0.9, 0.9));
    auto material_green_glass = make_shared<RefractiveMaterial>(color(0.92, 1.0, 0.92), 1.3);
    auto material_blue_glass = make_shared<RefractiveMaterial>(color(0.92, 0.92, 1.0), 1.3);
    auto material_bowl = make_shared<RefractiveMaterial>(color(0.8, 0.8, 0.8), 0.4);
    auto material_light = make_shared<EmissiveMaterial>(color(1.0, 1.0, 1.0), 5.0);

    world.add(make_shared<Plane>(point3(0.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0), material_ground));
    world.add(make_shared<Plane>(point3(0.0, 0.0, -3.0), vec3(0.0, 0.0, 1.0), material_wall));
    ...
    world.add(make_shared<Sphere>(point3(-3.0, 10.0, 8.0), 6.0, material_light));
